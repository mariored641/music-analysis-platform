...
### שלב 4.5 — ויזואליות קטגוריות annotations ✅ (הושלם מרץ 2026)
**מטרה:** כל קטגוריית תיוג מוצגת בצורה ויזואלית ייחודית ומשמעותית על הפרטיטורה.

**הרמוניה:**
- [x] טקסט מעל החמשה — overlay SVG, פונט serif, גודל/גובה תואם לאקורדי Verovio
- [x] עוגן בלתי נראה ברגיל; כשנבחר — קו מקוקו לתו העוגן + נקודת עוגן
- [x] Ctrl+חצים כשנבחר → מזיז את נקודת העוגן תו-תו
- [x] לחיצה על annotation פותחת תפריט הרמוניה ובוחרת תו עוגן
- [x] שקול לאקורד XML לסקריפטים (noteIds מכיל noteMapId של תו העוגן)

**מלודיה:**
- [x] ראש התו צבוע בצבע הקטגוריה (מחובר ל-legendColors בפאנל שמאל)
- [x] tooltip: "תו אקורד" / "מעבר" / "שכן"
- [x] צבעי ברירת מחדל: CT=כחול, PT=סגול, NT=ירוק, SUS=כתום, APP=ענבר

**מוטיב:**
- [x] Metaball blob — צורה אורגנית סביב ראשי התווים; fill שקוף; stroke בצבע המוטיב
- [x] רדיוס = ראש תו + מעט (R=13px)
- [x] ריצוף רציף גם מעל תווים שאינם חלק מהמוטיב
- [x] ריבוי שורות: נחתך קו ישר אנכי בסוף שורה, ממשיך בתחילת הבאה (leftCap/rightCap: 'flat'/'round')
- [x] תוקן: left cap sweep direction (arc 0→1 = עקומה החוצה)

**מבנה (Form):**
- [x] קו אופקי מתחת לכל החמשות + סמן אנכי בשני הקצוות
- [x] טקסט באמצע; מופיע שוב בכל שורה אם מתפרס
- [x] 2 עוגנים כשנבחר; Ctrl+חצים להזזה

**תוויות (Labels):**
- [x] ~14px מעל ראש כל תו — פוליליין זיגזג עוקב מלודיה
- [x] טקסט באמצע האופקי (textAnchor="middle"), לא בהתחלה
- [x] סמן אנכי בשני הקצוות; הפסקות — ממשיך ישר

**כללי:**
- [x] hover tooltip לכל annotation (סוג + טקסט אם יש)
- [x] כל הטאבים תמיד מוצגים בתפריט הקשר (Harm/Melody/Form/Motif/Label/Color/?), סדר לפי רלוונטיות
- [x] **תיקון scroll**: `elementMap` שומר עכשיו container-relative positions — annotations גוללים נכון עם הפרטיטורה
...
- תפריט הקשר: כל 7 טאבים תמיד נראים, סדר לפי רלוונטיות
- תיקון scroll: elementMap עם container-relative positions
- תיקון motif left cap arc sweep direction
- הרמוניה: לחיצה בוחרת תו עוגן + פותחת תפריט
